...
def bing_web_search(query, subscription_key, endpoint=None, market='en-US', language='en', timeout=20):
    """
    Perform a search using the Serper.dev API, but return results in the same
    shape as Bing Web Search so that the rest of the code can remain unchanged.

    Args:
        query (str): Search query.
        subscription_key (str): Serper.dev API key (we reuse the
                                --bing_subscription_key flag to pass this).
        endpoint (str): Ignored for Serper.dev (kept for compatibility).
        market (str): Ignored for Serper.dev (kept for compatibility).
        language (str): Ignored for Serper.dev (kept for compatibility).
        timeout (int or float): Request timeout in seconds.

    Returns:
        dict: JSON response shaped like Bing's search results:
              {
                "webPages": {
                  "value": [
                    {
    ...
    try:
        ...
        print(f"Serper.dev Web Search request timed out ({timeout} seconds) for query: {query}")
        return {}
    except requests.exceptions.RequestException as e:
        print(f"Error occurred during Serper.dev Web Search request: {e}")
        return {}

    webpages = []
    organic_results = data.get("organic", [])
    for item in organic_results:
        # Serper fields (may be missing):
