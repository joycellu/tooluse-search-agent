# run_search_o1.py
# Attempted to improve open already made search_o1 by
import os
import json
import time
import re
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="Starting index for the dataset slice."
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=-1,
        help="Ending index for the dataset slice. -1 means process to the end."
    )
...
def main():
    ...
    # Slice the dataset based on the provided start and end indices
    input_list = input_list[start_idx:end_idx]
    filtered_data = filtered_data[start_idx:end_idx]
    
    print(f"Processing slice: Items {start_idx} to {end_idx} (Total: {len(input_list)} items)")
